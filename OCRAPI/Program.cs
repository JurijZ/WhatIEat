...
namespace OCRAPI
{
    ...
    class Program
    {
        ...
        static readonly string _filename = "Test.JPG";  //"Sample.xml";
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                // Run HttpClient issuing requests (sends a file to the API asynchronously)
                RunClient();
        ...
        static async void RunClient()
        {
            ...
            //HttpClient client = HttpClientFactory.Create(progress); // When progress notification handler is used
            ...
            using (FileStream fileStream = new FileStream(_filename, FileMode.Open, FileAccess.Read, FileShare.Read, BufferSize, useAsync: true))
            {
                ...
                //Console.WriteLine("{0}Result:{0}  Filename:  {1}{0}  Submitter: {2}", Environment.NewLine, result.FileNames.FirstOrDefault(), result.RecognizedText);
